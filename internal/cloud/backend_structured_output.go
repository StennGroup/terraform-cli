...
func (b *Cloud) renderPlan(ctx context.Context, run *tfe.Run) error {
	...
	deferredLogs, err := b.renderLogs(ctx, run, logs)
	if err != nil {
		return err
	}
	...
	if ws.StructuredRunOutputEnabled && b.renderer != nil {
		...
		// We will only have deferred logs when JSON logging is enabled
...
func (b *Cloud) renderApply(ctx context.Context, run *tfe.Run) error {
	...
	// As of now, there are no logs that are deferred during
	// an apply operation.
	_, err = b.renderLogs(ctx, run, logs)
	if err != nil {
		return err
	}
	return nil
...
func (b *Cloud) renderLogs(ctx context.Context, run *tfe.Run, logs io.Reader) ([]jsonformat.JSONLog, error) {
	...
	if b.CLI != nil {
		for next := true; next; {
			...
			for isPrefix := true; isPrefix; {
				...
				if err != nil {
					if err != io.EOF {
						return deferredLogs, generalError("Failed to read logs", err)
					}
					next = false
				}
